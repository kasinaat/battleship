...
export default Route.extend({
  ...
  init(params){
    ...
    const socket = this.get('websockets').socketFor('ws://172.22.125.206:8080/battleship-server/websocket/'+localStorage.getItem('username')+'/'+params.game_id);
 ...
 myMessageHandler(event) {
   ...
   const socket = this.get('socketRef');
   var hitRegex = /^HIT#[0-9]#[A-z]+$/;
   var trueRegex = /^TRUE#[0-9]#[A-z]+$/;
   var falseRegex = /^FALSE#[0-9]#[A-z]+$/;
   ...
   if((message).includes("READY#")){
     ...
     if(this.controller.get('point') == 3){
       socket.send("WINNER#0#"+this.get("opponent"));
